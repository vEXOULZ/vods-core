...
| `api` | `ArchiveClient`: `listVods` (title / game / date filters, combinable), `getVod`, `liveStream`, `vodEmotes`, `badges`, `commentsAt` / `commentsAfter`. Normalizes VODs: chapter `end` becomes an absolute time (the API sends the length), `duration` becomes seconds. |
...
| `chat` | `ChatReplay` (paged, prefetching, seek-aware), `loadEmotes` (archive sets, else BTTV/FFZ/7TV channel sets, plus 7TV globals), `tokenize` / `resolveBadges` / `toChatMessage` (render-ready tokens, never HTML). |
